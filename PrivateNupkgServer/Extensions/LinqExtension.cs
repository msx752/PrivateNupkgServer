...
namespace privatenupkgserver.Extensions
{
    public static class LinqExtension
    {
        ...
        {
            return In<T>(@this, range, comparer);
        }

        public static bool In<T>(this T @this, IEnumerable<T> range, Func<T, T, bool> comparer = null)
        {
            return range?.Any(e => comparer?.Invoke(@this, e) ?? Equals(@this, e)) == true;
        }
        ...
        public static IDictionary<TKey, TValue> OfValueType<TKey, TValue>(this IDictionary<TKey, object> dictionary)
        {
            ...
            foreach (var value in allValues)
            {
                ...
                foreach (var key in dictionary.Keys)
                {
                    ...
                    if (ReferenceEquals(objectValue, value) || Equals(objectValue, value))
                    {
                        dic.Add(key, value);
                    }
                }
        ...
        public static T FirstOrDefault<T>(this IDictionary dictionary, string key)
        {
            ...
            return dictionary.OfKeyType<string>().OfValueType<string, T>()
                .FirstOrDefault(x => string.Equals(key, x.Key, StringComparison.InvariantCultureIgnoreCase)).Value;
        }
    }
}